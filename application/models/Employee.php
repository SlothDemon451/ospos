...
class Employee extends Person
{
	...
	*/
	public function get_all($limit = 10000, $offset = 0, $delivery_only = false)
	{
		$this->db->from('employees');
		$this->db->where('deleted', 0);
		if ($delivery_only) {
			$this->db->where('is_delivery_man', 1);
		} else {
			$this->db->where('is_delivery_man', 0); // Exclude delivery men from regular employees
		}
	...
	*/
	public function get_search_suggestions($search, $include_deleted = FALSE, $limit = 5)
	{
		...
		$this->db->where('is_delivery_man', 0); // Exclude delivery men
		$this->db->order_by('last_name', 'asc');
	...
	*/
	...
	/*
	Get search suggestions to find delivery men
	*/
	public function get_delivery_man_search_suggestions($search, $include_deleted = FALSE, $limit = 5)
	{
		$suggestions = array();
	...
	/*
	Gets rows for delivery men search
	*/
	public function get_found_delivery_men_rows($search)
	{
		return $this->search_delivery_men($search, 0, 0, 'last_name', 'asc', TRUE);
	}

	/*
	Performs a search on delivery men
	*/
	public function search_delivery_men($search, $rows = 0, $limit_from = 0, $sort = 'last_name', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		if($count_only == TRUE)
		{
	...
	/*
	Deletes a list of delivery men
	*/
	public function delete_delivery_men_list($delivery_man_ids)
	{
		$success = FALSE;

		//Don't let employees delete theirself
		if(in_array($this->get_logged_in_employee_info()->person_id, $delivery_man_ids))
		{
			return FALSE;
		}

		// Check if any delivery men have completed or suspended sales
		$this->db->select('COUNT(*) as count');
		$this->db->from('sales');
		$this->db->where_in('delivery_man_id', $delivery_man_ids);
		$this->db->where_in('sale_status', array(COMPLETED, SUSPENDED));
		$query = $this->db->get();
		$result = $query ? $query->row() : null;
		
		$sales_count = $result ? $result->count : 0;

		if($sales_count > 0)
		{
			return FALSE; // Cannot delete delivery men with sales
		}

		//Run these queries as a transaction, we want to make sure we do all or nothing
		$this->db->trans_start();

		// Delete permissions from grants table
		$this->db->where_in('person_id', $delivery_man_ids);
	...
	/*
	Check if delivery men can be deleted (no sales)
	*/
	public function can_delete_delivery_men($delivery_man_ids)
	{
		// First verify these are actually delivery men
		$this->db->select('employees.person_id');
		$this->db->from('employees');
		$this->db->where_in('employees.person_id', $delivery_man_ids);
		$this->db->where('employees.is_delivery_man', 1);
		$this->db->where('employees.deleted', 0);
		$valid_delivery_men = $this->db->get()->result();
		
		if(empty($valid_delivery_men)) {
			return FALSE;
		}

				// Check if there are any completed or suspended sales with these delivery_man_ids
		$this->db->select('COUNT(*) as count');
		$this->db->from('sales');
		$this->db->where_in('delivery_man_id', $delivery_man_ids);
		$this->db->where_in('sale_status', array(COMPLETED, SUSPENDED));
