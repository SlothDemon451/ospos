<?php $is_embed = isset($embed) && $embed ? TRUE : FALSE; ?>
<?php if(!$is_embed) { $this->load->view("partial/header"); } ?>
...
<?php if(!$is_embed) { $this->load->view('partial/print_receipt', array('print_after_sale'=>$print_after_sale, 'selected_printer'=>'invoice_printer')); } ?>
...
<div id="receipt_wrapper" style="font-size:<?php echo $this->config->item('receipt_font_size');?>px; max-width: 900px; margin: auto;">
	<!-- Header: Invoice and Client -->
	<table class="suspended-header-table">
		<tr>
			<th style="width:50%;">Factura:</th>
			<th style="width:50%;">Cliente:</th>
		</tr>
		<tr>
			<td>
				<div class="suspended-info-block">
					Num: <?php echo $invoice_number; ?><br>
					Fecha: <?php echo $transaction_date; ?><br>
			...
			<td>
				<div class="suspended-info-block">
					...
					<?php endif; ?>
					<?php if(isset($first_name) && isset($last_name) && !empty($first_name . ' ' . $last_name)): ?>
						<?php echo htmlspecialchars($first_name . ' ' . $last_name); ?><br>
					<?php endif; ?>
					<?php if(isset($customer_address) && !empty($customer_address)): ?>
						<?php echo htmlspecialchars($customer_address); ?><br>
					<?php endif; ?>
					<?php if(isset($customer_location) && !empty($customer_location)): ?>
						<?php echo htmlspecialchars($customer_location); ?><br>
					<?php endif; ?>
					<?php if(isset($customer_phone) && !empty($customer_phone)): ?>
						Tel: <?php echo htmlspecialchars($customer_phone); ?><br>
					<?php endif; ?>
					<?php if(isset($customer_email) && !empty($customer_email)): ?>
						Email: <?php echo htmlspecialchars($customer_email); ?><br>
					<?php endif; ?>
					<?php if(isset($customer_account_number) && !empty($customer_account_number)): ?>
						Cuenta: <?php echo htmlspecialchars($customer_account_number); ?><br>
					<?php endif; ?>
					<?php if(isset($tax_id) && !empty($tax_id)): ?>
						Tax ID: <?php echo htmlspecialchars($tax_id); ?>
					<?php endif; ?>
				</div>
			</td>
		</tr>
...
<?php if(!$is_embed) { $this->load->view("partial/footer"); } ?>
