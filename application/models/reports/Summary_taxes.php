...
class Summary_taxes extends Summary_report
{
	...
	public function getData(array $inputs)
	{
		...
		$sql = "SELECT name as name, percent, COUNT(DISTINCT sale_id) AS count, ROUND(SUM(subtotal), $decimals) AS subtotal, ROUND(SUM(tax), $decimals) AS tax, ROUND(SUM(total), $decimals) AS total
			FROM (
				SELECT
					name AS name,
		...
		if($query === FALSE)
		{
			error_log("Summary_taxes query failed: " . $this->db->last_query());
			return array();
		}
