...
class Items extends Secure_Controller
{
	...
	public function search()
	{
		...
		$definition_names = $this->ItemAttribute->get_definitions_by_flags(ItemAttribute::SHOW_IN_ITEMS);
	...
	public function view($item_id = NEW_ITEM)
	{
		...
		$categories = array('' => $this->lang->line('items_none'));
		foreach($this->Item_category->get_all()->result() as $cat) {
			$categories[$cat->id] = $cat->name;
		}
		$data['categories'] = $categories;
		$data['selected_category_id'] = $item_info->item_category_id;

		$subcategories = array('' => $this->lang->line('items_none'));
		if ($item_info->item_category_id) {
			foreach($this->Item_subcategory->get_by_category($item_info->item_category_id)->result() as $subcat) {
				$subcategories[$subcat->id] = $subcat->name;
			}
		}
		$data['subcategories'] = $subcategories;
		$data['selected_subcategory_id'] = $item_info->item_subcategory_id;
	...
	public function save($item_id = NEW_ITEM)
	{
		...
		$item_data = array(
			...
			'pic_filename' => $upload_data['orig_name'],
		);
		...
		if($this->Item->save($item_data, $item_id))
		{
			...
			if(!$use_destination_based_tax)
			{
				...
				$tax_types = $this->input->post('tax_types');
				...
				foreach($tax_percents as $tax_percent)
				{
					...
					$tax_type = (isset($tax_types) && is_array($tax_types) && in_array($tax_name_index + 1, $tax_types)) ? 1 : 0;

					if(is_numeric($tax_percentage))
					{
						$items_taxes_data[] = array(
							'name' => $tax_names[$tax_name_index], 
	...
	public function bulk_update()
	{
		...
		if(empty($item_data) || $this->Item->update_multiple($item_data, $items_to_update))
		{
			...
			foreach($tax_percents as $tax_percent)
			{
				...
				{
					$tax_updated = TRUE;
					$tax_type = (isset($tax_types) && is_array($tax_types) && in_array($tax_percent + 1, $tax_types)) ? 1 : 0;
					$items_taxes_data[] = array(
						'name' => $tax_names[$tax_percent], 
						'percent' => $tax_percents[$tax_percent],
						'tax_type' => $tax_type
					);
				}
			}
	...
	private function save_tax_data($row, $item_data)
	{
		...
		{
			$tax_type = isset($row['Tax 1 Type']) && $row['Tax 1 Type'] == 'Fixed' ? 1 : 0;
			$items_taxes_data[] = array(
				'name' => $row['Tax 1 Name'], 
				'percent' => $row['Tax 1 Percent'],
				'tax_type' => $tax_type
			);
		}
		...
		{
			$tax_type = isset($row['Tax 2 Type']) && $row['Tax 2 Type'] == 'Fixed' ? 1 : 0;
			$items_taxes_data[] = array(
				'name' => $row['Tax 2 Name'], 
				'percent' => $row['Tax 2 Percent'],
				'tax_type' => $tax_type
			);
		}
	...
	{
		$this->load->model('Item_subcategory');
		$subcategories = $this->Item_subcategory->get_by_category($category_id)->result();
		$result = array();
	...
	// Returns all categories as JSON for cascading select
	public function get_all_categories() {
		$this->load->model('Item_category');
		$categories = $this->Item_category->get_all(0, 0, TRUE)->result();
		$result = array();
	...
	// Returns all subcategories for a category as JSON for cascading select
	public function get_subcategories_json($category_id) {
		$this->load->model('Item_subcategory');
		$subcategories = $this->Item_subcategory->get_by_category($category_id)->result();
		$result = array();
		foreach($subcategories as $subcat) {
	...
	// Returns all items for a subcategory as JSON for cascading select
	public function get_items_by_subcategory($subcategory_id) {
		$this->load->model('Item');
		$this->db->from('items');
		$this->db->where('item_subcategory_id', $subcategory_id);
	...
	// Returns all items for a category as JSON for cascading select
