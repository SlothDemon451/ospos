...
class Customers extends Persons
{
	...
	*/
	...
	public function sales($customer_id)
	{
		...
		// Get all sales for this customer, optionally filtered by date
		$sales = $this->Sale->get_sales_by_customer($customer_id, $start_date, $end_date);
		...
		$data['title'] = 'Sales for ' . $customer_info->first_name . ' ' . $customer_info->last_name;

		$this->load->view('sales/manage', $data);
	}

	public function apply_payment($customer_id)
	{
		$this->load->model('Sale');
		...
		if ($this->input->method() === 'post') {
			...
			// Fetch all unpaid sales for this customer, oldest first
			...
			foreach ($unpaid_sales as $sale) {
				...
				if ($pay > 0) {
					...
					$this->Sale->add_payment_to_sale($sale->sale_id, [
			...
			$this->load->view('customers/payment_result', $data);
			return;
		}
