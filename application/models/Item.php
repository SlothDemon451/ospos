...
class Item extends CI_Model
{
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		try {
			...
			else
			{
				...
				$this->db->select('MAX(ospos_item_categories.name) AS category');
			...
			if(!empty($search))
			{
				...
				else
				{
					$this->db->group_start();
						...
						$this->db->or_like('ospos_item_categories.name', $search);
					$this->db->group_end();
				}
			}
			...
			if ($items === false) {
				$error = $this->db->error();
				die('<pre>SQL ERROR: ' . print_r($error, true) . "\nLast Query: " . $this->db->last_query() . '</pre>');
			}
	...
	*/
	public function get_info($item_id)
	{
		...
		$this->db->select('ospos_item_categories.name AS category');
		...
		else
		{
			...
			// Ensure tax_option is always set
			$item_obj->tax_option = 'without_tax';
	...
	*/
	...
	public function get_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE), $unique = FALSE, $limit = 25)
	{
		...
		if(!$unique)
		{
			...
			$this->db->join('ospos_item_categories', 'ospos_item_categories.id = items.item_category_id', 'left');
			$this->db->where('items.deleted', $filters['is_deleted']);
			$this->db->where_in('items.item_type', $non_kit);
	...
	public function get_stock_search_suggestions($search, $filters = array('is_deleted' => FALSE, 'search_custom' => FALSE), $unique = FALSE, $limit = 25)
	{
		...
		{
			//Search by category
			$this->db->select('ospos_item_categories.name');
			$this->db->from('items');
			$this->db->where('deleted', $filters['is_deleted']);
			$this->db->where_in('item_type', $non_kit); // standard, exclude kit items since kits will be picked up later
			$this->db->where('stock_type', '0'); // stocked items only
			$this->db->distinct();
			$this->db->like('ospos_item_categories.name', $search);
			$this->db->order_by('ospos_item_categories.name', 'asc');
			foreach($this->db->get()->result() as $row)
			{
