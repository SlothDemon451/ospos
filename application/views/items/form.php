...
<?php echo form_open('items/save/'.$item_info->item_id, array('id'=>'item_form', 'enctype'=>'multipart/form-data', 'class'=>'form-horizontal')); ?>
	<fieldset id="item_basic_info">
		...
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('item_categories_name'), 'item_category_id', array('class'=>'required control-label col-xs-3')); ?>
			<div class='col-xs-8'>
				<?php echo form_dropdown('item_category_id', $categories, isset($selected_category_id) ? $selected_category_id : '', 'class="form-control" id="item_category_id"'); ?>
			</div>
		</div>
		<div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('item_subcategories_name'), 'item_subcategory_id', array('class'=>'control-label col-xs-3')); ?>
			<div class='col-xs-8'>
				<?php echo form_dropdown('item_subcategory_id', $subcategories, isset($selected_subcategory_id) ? $selected_subcategory_id : '', 'class="form-control" id="item_subcategory_id"'); ?>
			</div>
		</div>
		...
		<!-- Add tax option radio buttons -->
		<!-- <div class="form-group form-group-sm">
			<?php echo form_label('Tax Price', 'tax_option', array('class'=>'control-label col-xs-3')); ?>
			<div class="col-xs-8">
				<label class="radio-inline">
					<?php echo form_radio(array(
							'name'=>'tax_option',
							'type'=>'radio',
							'id'=>'tax_option_without',
							'value'=>'without_tax',
							'checked'=>!isset($item_info->tax_option) || $item_info->tax_option != 'with_tax',
							'onclick'=>'enableTaxFields()'
					)); ?> Without Tax
				</label>
				<label class="radio-inline">
					<?php echo form_radio(array(
			...
			<div class="form-group form-group-sm">
				...
				<div class="col-xs-2">
					<label class="control-label">
						<input type="checkbox" id="tax_type_1" name="tax_types[]" value="1" onclick="toggleTaxType(1)" <?php echo isset($item_tax_info[0]['tax_type']) && $item_tax_info[0]['tax_type'] == 1 ? 'checked' : ''; ?>>
						Fixed
					</label>
				</div>
				<div class="col-xs-12">
					<span style="color:red; font-weight: bold;">Price with tax: </span><span id="price_with_tax"></span>
				</div>
				<div class="col-xs-6">
					<span style="color:blue; font-weight: bold;">Base price (before tax): </span><span id="base_price"></span>
				</div>
				<div class="col-xs-6">
					<span style="color:blue; font-weight: bold;">Tax Amount: </span><span id="tax_amount"></span>
				</div>
			</div>
<!-- 
			<div class="form-group form-group-sm">
				...
				<div class="col-xs-3">
					<div class="input-group input-group-sm">
						...
						<span class="input-group-addon input-sm tax-percent-symbol"><b><?php echo isset($item_tax_info[1]['tax_type']) && $item_tax_info[1]['tax_type'] == 1 ? $this->config->item('currency_symbol') : '%'; ?></b></span>
					</div>
				</div>
				<div class="col-xs-2">
					<label class="control-label">
						<input type="checkbox" id="tax_type_2" name="tax_types[]" value="2" onclick="toggleTaxType(2)" <?php echo isset($item_tax_info[1]['tax_type']) && $item_tax_info[1]['tax_type'] == 1 ? 'checked' : ''; ?>>
						Fixed
					</label>
				</div>
			</div> -->
...
$(document).ready(function()
{
	function calculatePriceWithTax() {
        ...
        var taxPercent = parseFloat($('#tax_percent_name_1').val()) || 0; // Assuming you're using the first tax input
        var isFixed = $('#tax_type_1').is(':checked'); // Check if the first tax type is fixed

        var priceWithTax;
        var basePrice;

        if (isFixed) {
            // If fixed, the unit price is the final price (including tax)
            // Calculate the base price (before tax) for reference
            basePrice = unitPrice / (1 + (taxPercent / 100));
            priceWithTax = unitPrice; // Show the retail price as price with tax
            $('#base_price').text(basePrice.toFixed(2));
            $('#tax_amount').text((unitPrice - basePrice).toFixed(2));
            $('#base_price').parent().show(); // Show base price row
			$('#tax_amount').parent().show(); // Show tax amount row
            $('#price_with_tax').parent().hide(); // Hide the price with tax row
            // Always show % symbol when fixed is checked
            $('#tax_percent_name_1').next('.tax-percent-symbol').find('b').text('%');
        } else {
            // If percentage, calculate the price with tax
            priceWithTax = unitPrice + (unitPrice * (taxPercent / 100));
            $('#tax_amount').text((unitPrice - basePrice).toFixed(2));
            $('#base_price').text(''); // Clear base price if not fixed
            $('#base_price').parent().hide(); // Hide base price row
			$('#tax_amount').parent().hide(); // Clear tax amount if not fixed
            $('#price_with_tax').parent().show(); // Show the price with tax row
            // Set the symbol based on the checkbox state
            if ($('#tax_type_1').is(':checked')) {
                $('#tax_percent_name_1').next('.tax-percent-symbol').find('b').text('%');
            } else {
                $('#tax_percent_name_1').next('.tax-percent-symbol').find('b').text('%');
            }
        ...
        $('#price_with_tax').text(priceWithTax ? priceWithTax.toFixed(2) : ''); 
    }

    // Event listeners to recalculate price with tax
    $('#unit_price').on('input', calculatePriceWithTax);
    $('#tax_percent_name_1').on('input', calculatePriceWithTax); 
    $('#tax_type_1').on('change', calculatePriceWithTax);

	calculatePriceWithTax();
	// Function to enable tax fields
	window.enableTaxFields = function() {
		$('#tax_name_1').prop('disabled', false);
		$('#tax_percent_name_1').prop('disabled', false);
		$('#tax_name_2').prop('disabled', false);
		$('#tax_percent_name_2').prop('disabled', false);
		$('#tax_type_1').prop('disabled', false);
		$('#tax_type_2').prop('disabled', false);
	}

	// Function to disable tax fields
	window.disableTaxFields = function() {
		// Clear the fields
		$('#tax_name_1').val('').prop('disabled', true);
		$('#tax_percent_name_1').val('').prop('disabled', true);
		$('#tax_name_2').val('').prop('disabled', true);
		$('#tax_percent_name_2').val('').prop('disabled', true);
		
		// Uncheck and disable the tax type checkboxes
		$('#tax_type_1').prop('checked', false).prop('disabled', true);
		$('#tax_type_2').prop('checked', false).prop('disabled', true);
		
		// Reset the symbols to %
		$('.tax-percent-symbol').find('b').text('%');
	}

	// Function to toggle between percentage and fixed amount
	window.toggleTaxType = function(tax_id) {
		var is_fixed = $('#tax_type_' + tax_id).is(':checked');
		if (is_fixed) {
			$('#tax_percent_name_' + tax_id).next('.tax-percent-symbol').find('b').text('%');
		} else {
			$('#tax_percent_name_' + tax_id).next('.tax-percent-symbol').find('b').text('%');
		}
		calculatePriceWithTax();
	...
	// Initialize tax fields based on the selected tax option
	if ($('#tax_option_with').is(':checked')) {
		disableTaxFields();
	} else {
		enableTaxFields();
		
		// Initialize tax symbols based on existing tax types
		<?php if(isset($item_tax_info[0]['tax_type']) && $item_tax_info[0]['tax_type'] == 1): ?>
			$('#tax_type_1').prop('checked', true);
			$('#tax_percent_name_1').next('.tax-percent-symbol').find('b').text('<?php echo $this->config->item('currency_symbol'); ?>');
		<?php endif; ?>
		
		<?php if(isset($item_tax_info[1]['tax_type']) && $item_tax_info[1]['tax_type'] == 1): ?>
			$('#tax_type_2').prop('checked', true);
			$('#tax_percent_name_2').next('.tax-percent-symbol').find('b').text('<?php echo $this->config->item('currency_symbol'); ?>');
		<?php endif; ?>
	}
	...
	$('#item_category_id').change(function() {
		...
		if(category_id) {
			$.getJSON('items/get_subcategories/' + category_id, function(data) {
				var $subcat = $('#item_subcategory_id');
				$subcat.empty();
				$.each(data, function(key, value) {
					$subcat.append($('<option></option>').attr('value', key).text(value));
				});
			});
		} else {
	...
	var init_validation = function() {
		$('#item_form').validate($.extend({
			submitHandler: function(form, event) {
				// Clear tax data if "with_tax" option is selected
				if ($('#tax_option_with').is(':checked')) {
					// Remove any existing tax inputs
					$('input[name="tax_names[]"]').remove();
					$('input[name="tax_percents[]"]').remove();
					
					// Add empty hidden fields for tax data to ensure they're cleared in the database
					$('<input>').attr({
						type: 'hidden',
						name: 'tax_names[]',
						value: ''
			...
			messages:
			{
				...
				item_category_id: "<?php echo $this->lang->line('items_category_required'); ?>",
				cost_price:
				{
