...
class Sale extends CI_Model
{
	...
	public function get_info($sale_id)
	{
		...
		$this->db->select('
				...
				MAX(sales.delivery_man_id) AS delivery_man_id,
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		// Modified sale price calculation to handle fixed tax correctly
		$sale_price = 'CASE WHEN sales_items.discount_type = ' . PERCENT
			. " THEN sales_items.quantity_purchased * sales_items.item_unit_price - ROUND(sales_items.quantity_purchased * sales_items.item_unit_price * sales_items.discount / 100, $decimals) "
			. 'ELSE sales_items.quantity_purchased * (sales_items.item_unit_price - sales_items.discount) END';
	 ...
	 */
	public function save($sale_id, &$sale_status, &$items, $customer_id, $employee_id, $delivery_man_id, $comment, $invoice_number,
	...
	{
		log_message('debug', 'Starting delete for sale_id: ' . $sale_id . ', employee_id: ' . $employee_id . ', update_inventory: ' . ($update_inventory ? 'true' : 'false'));
		
		// start a transaction to assure data integrity
		$this->db->trans_start();

		$sale_status = $this->get_sale_status($sale_id);
		log_message('debug', 'Sale status for sale_id ' . $sale_id . ': ' . $sale_status);
		...
		if($update_inventory && $sale_status == COMPLETED)
		{
			...
			log_message('debug', 'Found ' . count($items) . ' items for sale_id: ' . $sale_id);
			
			foreach($items as $item)
			{
				log_message('debug', 'Processing item_id: ' . $item['item_id'] . ' for sale_id: ' . $sale_id);
				$cur_item_info = $this->Item->get_info($item['item_id']);

				if(!$cur_item_info) {
					log_message('error', 'Item info not found for item_id: ' . $item['item_id'] . ' in sale_id: ' . $sale_id);
					continue;
				}
				...
				if($cur_item_info->stock_type == HAS_STOCK)
				{
					...
					// update inventory
					$inventory_result = $this->Inventory->insert($inv_data);
					if(!$inventory_result) {
						log_message('error', 'Inventory insert failed for item_id: ' . $item['item_id'] . ' in sale_id: ' . $sale_id);
					}

					// update quantities
					$quantity_result = $this->Item_quantity->change_quantity($item['item_id'], $item['item_location'], $item['quantity_purchased']);
					if(!$quantity_result) {
						log_message('error', 'Quantity update failed for item_id: ' . $item['item_id'] . ' in sale_id: ' . $sale_id);
					}
				}
			}
		}
		...
		log_message('debug', 'Updated sale status to CANCELED for sale_id: ' . $sale_id);

		// execute transaction
		$this->db->trans_complete();

		if (!$this->db->trans_status()) {
			log_message('error', 'Sale delete transaction failed for sale_id: ' . $sale_id . ' - DB error: ' . print_r($this->db->error(), true));
		} else {
			log_message('debug', 'Sale delete successful for sale_id: ' . $sale_id);
		}
	...
	/**
	 * Recalculate sale totals for fixed tax items to display correctly in manage view
	 */
	public function recalculate_fixed_tax_totals($sale_data)
	{
		$sale_id = $sale_data->sale_id;
		
		// Get all items and their taxes for this sale
		$this->db->select('
			sales_items.item_id,
			...
			sales_items_taxes.tax_type,
			sales_items_taxes.percent as tax_percent,
			sales_items_taxes.item_tax_amount
		');
		$this->db->from('sales_items');
		$this->db->join('sales_items_taxes', 'sales_items.sale_id = sales_items_taxes.sale_id AND sales_items.item_id = sales_items_taxes.item_id AND sales_items.line = sales_items_taxes.line', 'left');
		$this->db->where('sales_items.sale_id', $sale_id);
		...
		foreach($items as $item) {
			...
			if($discount_type == PERCENT) {
				$item_total = $quantity * $unit_price - ($quantity * $unit_price * $discount / 100);
			} else {
				$item_total = $quantity * ($unit_price - $discount);
			}
			
			if($item->tax_type == 1) { // Fixed tax
				$has_fixed_tax = true;
				$tax_percent = $item->tax_percent;
				// For fixed tax, the item_total includes tax, so calculate base price
				$base_price = $item_total / (1 + ($tax_percent / 100));
				$tax_amount = $item_total - $base_price;
				
				$corrected_subtotal += $base_price;
				$corrected_tax += $tax_amount;
				$corrected_total += $item_total; // Total is the retail price (including tax)
			} else {
				// Regular percentage tax
				$corrected_subtotal += $item_total;
				$item_tax = isset($item->item_tax_amount) ? $item->item_tax_amount : 0;
				$corrected_tax += $item_tax;
				$corrected_total += $item_total + $item_tax;
			}
		}
		
		// Update the display values (for both fixed and regular tax)
		if($has_fixed_tax) {
			// Fixed tax: subtotal is base price, total is retail price
			$sale_data->subtotal = number_format($corrected_subtotal, 2);
			$sale_data->total = number_format($corrected_total, 2);
		} else {
			// Regular tax: total is subtotal + tax
			$corrected_total = $corrected_subtotal + $corrected_tax;
			$sale_data->subtotal = number_format($corrected_subtotal, 2);
			$sale_data->total = number_format($corrected_total, 2);
		}
		
		// Always set the tax field for the new Tax column
		$sale_data->tax = number_format($corrected_tax, 2);
		
		// Calculate Amount Due = Total - Amount Tendered (outstanding balance)
		$amount_tendered = floatval(str_replace(',', '', $sale_data->amount_tendered));
		$total_amount = floatval(str_replace(',', '', $sale_data->total));
		$sale_data->amount_due = number_format($total_amount - $amount_tendered, 2);
		
		// Change Due = Amount Tendered - Total (if customer overpaid)
		$sale_data->change_due = number_format($amount_tendered - $total_amount, 2);
		
		return $sale_data;
	}

	public function get_sales_by_customer($customer_id, $start_date = null, $end_date = null)
	{
		$this->db->from('sales');
		$this->db->where('customer_id', $customer_id);
		$this->db->where('sale_status !=', CANCELED);
		if ($start_date) {
			$this->db->where('DATE(sale_time) >=', $start_date);
		}
		if ($end_date) {
			$this->db->where('DATE(sale_time) <=', $end_date);
		}
		...
		foreach ($sales as &$sale) {
			...
			$this->db->select('first_name, last_name');
			$this->db->from('people');
			$this->db->where('person_id', $sale->customer_id);
			$person = $this->db->get()->row();
			$sale->customer_name = $person ? $person->first_name . ' ' . $person->last_name : '';
			...
			$payment_types = array();
			foreach ($payments as $payment) {
				$sale->amount_tendered += $payment->payment_amount;
				$payment_types[] = $payment->payment_type . ' ' . to_currency($payment->payment_amount - $payment->cash_refund);
			}
	...
	public function get_sales_by_delivery_man($delivery_man_id, $start_date = null, $end_date = null)
	{
		$this->db->from('sales');
		$this->db->where('delivery_man_id', $delivery_man_id);
		$this->db->where('sale_status !=', CANCELED);
		if ($start_date) {
			$this->db->where('DATE(sale_time) >=', $start_date);
		}
	...
	// Returns all sales for a customer with outstanding dues, ordered by sale_time
	public function get_unpaid_sales($customer_id)
	{
		$completed_status = defined('COMPLETED') ? COMPLETED : '0';
		$this->db->from('sales');
		$this->db->where('customer_id', $customer_id);
		$this->db->where('sale_status', $completed_status);
		$this->db->order_by('sale_time', 'asc');
		$sales = $this->db->get()->result();

		$unpaid_sales = array();
		foreach ($sales as $sale) {
			// Calculate subtotal, tax, total as in get_sales_by_customer
			$this->db->from('sales_items');
			$this->db->where('sale_id', $sale->sale_id);
			$sale_items = $this->db->get()->result();
			$subtotal = 0;
