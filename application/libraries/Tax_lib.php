...
class Tax_lib
{
	 ...
	 */
    public function get_item_sales_tax($quantity, $price, $discount, $discount_type, $tax_percentage, $rounding_code, $tax_type = 0)
	{
		$decimals = tax_decimals();

		// Get the item total with discount applied
        $tax_basis = $this->CI->sale_lib->get_item_total($quantity, $price, $discount, $discount_type, TRUE);

		// If tax_type is 1, it's a fixed amount tax, not a percentage
		if($tax_type == 1)
		{
            // If tax is included in the price, do not add tax on top
            if($this->CI->config->item('tax_included'))
            {
                return 0;
            }
            // Apply discount to derive an effective unit price, then split into base and tax
            $effective_total = $this->CI->sale_lib->get_item_total($quantity, $price, $discount, $discount_type, TRUE);
            $effective_unit = ($quantity > 0) ? bcdiv($effective_total, $quantity, 4) : $price;
            $taxPercent = $tax_percentage;
            $basePrice = bcdiv($effective_unit, bcadd(1, bcdiv($taxPercent, 100, 6), 6), 6);
            $taxPerUnit = bcsub($effective_unit, $basePrice, 6);
            $taxAmountTotal = bcmul($taxPerUnit, $quantity, $decimals);
            return Rounding_mode::round_number($rounding_code, $taxAmountTotal, $decimals);
		}
		else
		{
			// For percentage tax, if tax is included in the price, we need to first extract the pre-tax amount
			if($this->CI->config->item('tax_included'))
			{
				// Calculate the pretax amount using the formula: pretax = price / (1 + rate/100)
				$tax_multiplier = bcadd(1, bcdiv($tax_percentage, 100));
				$pretax_basis = bcdiv($tax_basis, $tax_multiplier);
				
				// Calculate the tax amount based on the pretax amount
				$tax_amount = bcsub($tax_basis, $pretax_basis);
				
				// Round the tax amount to ensure consistency
				return Rounding_mode::round_number($rounding_code, $tax_amount, $decimals);
			}
			else
			{
				// If tax is not included, calculate it based on the total
	...
	{
		// If tax is included in the price, we need to first extract the pre-tax amount
		if($this->CI->config->item('tax_included'))
		{
			// Calculate the pretax amount using the formula: pretax = price / (1 + rate/100)
			$tax_multiplier = bcadd(1, bcdiv($tax_percentage, 100));
			...
			// Calculate the tax amount as the difference between the total and pretax amount
			$tax_amount = bcsub($tax_basis, $pretax_basis);
		}
		else
		{
			// If tax is not included, calculate it directly as a percentage of the basis
			...
			foreach($cart as $line => $item)
			{
				...
				if(!($this->CI->config->item('use_destination_based_tax')))
				{
					...
					foreach($tax_info as $tax)
					{
						...
						// Check if tax_type is defined, defaulting to 0 (percentage) if not
						$tax_type = isset($tax['tax_type']) ? $tax['tax_type'] : 0;
						
						// Save original tax_type for later use
						$original_tax_type = $tax_type;

                    // Treat tax as included either by global setting or by item-level tax_option "with_tax"
                    $treat_included = $this->CI->config->item('tax_included') || (isset($item['tax_option']) && $item['tax_option'] === 'with_tax');

                    if($treat_included)
						{
							$tax_type = Tax_lib::TAX_TYPE_INCLUDED;
							$tax_amount = $this->get_included_tax($item['quantity'], $item['price'], $item['discount'], $item['discount_type'], $tax['percent'], $tax_decimals, Rounding_mode::HALF_UP, $original_tax_type);
						}
						else
						{
							$tax_type = Tax_lib::TAX_TYPE_EXCLUDED;
							$tax_amount = $this->get_item_sales_tax($item['quantity'], $item['price'], $item['discount'], $item['discount_type'], $tax['percent'], Rounding_mode::HALF_UP, $original_tax_type);
						}
						...
						$items_taxes_detail['tax_type'] = $original_tax_type;
    ...
    public function get_included_tax($quantity, $price, $discount_percentage, $discount_type, $tax_percentage, $tax_decimal, $rounding_code, $tax_type = 0)
	{
		// If tax_type is 1, it's a fixed amount tax, not a percentage
		if($tax_type == 1)
		{
            // For fixed tax represented as an included percentage, split effective unit price into base and tax
            $effective_total = $this->CI->sale_lib->get_item_total($quantity, $price, $discount_percentage, $discount_type, TRUE);
            $effective_unit = ($quantity > 0) ? bcdiv($effective_total, $quantity, 4) : $price;
            $tax_multiplier = bcadd(1, bcdiv($tax_percentage, 100, 6), 6);
            $base = bcdiv($effective_unit, $tax_multiplier, 6);
            $tax_per_unit = bcsub($effective_unit, $base, 6);
            $tax_amount = bcmul($tax_per_unit, $quantity, $tax_decimal);
            return Rounding_mode::round_number($rounding_code, $tax_amount, $tax_decimal);
		}
		else
		{
			// For percentage tax, calculate using the standard formula
			$item_total = $this->CI->sale_lib->get_item_total($quantity, $price, $discount_percentage, $discount_type, TRUE);
			
			// Calculate the pretax amount using the formula: pretax = total / (1 + rate/100)
			$tax_multiplier = bcadd(1, bcdiv($tax_percentage, 100));
			$pretax_amount = bcdiv($item_total, $tax_multiplier);
			
			// The tax amount is the difference between the total and pretax amount
			$tax_amount = bcsub($item_total, $pretax_amount);
	...
	*/
	...
	{
		// Get the original tax_type from the item_taxes configuration if available
		$original_tax_type = $tax_type;
		
		// Try to find the original tax type for this tax from the database
		if($sale_id == -1) // New sale, not a loaded one
		{
			$this->CI->db->select('tax_type');
			$this->CI->db->from('ospos_items_taxes');
			...
			{
				$result = $query->row();
				$original_tax_type = $result->tax_type;
			}
		}
		...
		if(!array_key_exists($tax_group_index, $taxes))
		{
			...
			$tax = array($insertkey => array(
				'sale_id' => $sale_id,
				'tax_type' => $original_tax_type, // Use the original tax_type
				'tax_group' => $tax_group,
	...
	public function apply_destination_tax(&$item, $city, $state, $sales_tax_code_id, $register_mode, $sale_id, &$taxes, &$item_taxes, $line)
	{
		...
		if($tax_code_id != -1 && $item['price'] != 0)
		{
			...
			foreach($tax_definition as $tax)
			{
				...
				{
					$tax_amount = $this->get_included_tax($item['quantity'], $item['price'], $item['discount'], $item['discount_type'], $tax_rate, $tax_decimals, $rounding_code, $tax_type);
				}
				else
				{
