...
class Sales extends Secure_Controller
{
	...
	public function search()
	{
		...
		if (empty($filters['start_date']) || empty($filters['end_date'])) {
			$filters['start_date'] = date('Y-m-d 00:00:00', strtotime('-7 days'));
			$filters['end_date'] = date('Y-m-d 23:59:59');
		} else {
			// Try to parse as MM/DD/YYYY first, then as YYYY-MM-DD
			$start = DateTime::createFromFormat('m/d/Y', $filters['start_date']);
			$end = DateTime::createFromFormat('m/d/Y', $filters['end_date']);
			if (!$start) {
			...
			if ($start && $end) {
				$filters['start_date'] = $start->format('Y-m-d 00:00:00');
				$filters['end_date'] = $end->format('Y-m-d 23:59:59');
			} else {
				// fallback to previous logic if parsing fails
				$filters['start_date'] = date('Y-m-d 00:00:00', strtotime(str_replace('/', '-', $filters['start_date'])));
				$filters['end_date'] = date('Y-m-d 23:59:59', strtotime(str_replace('/', '-', $filters['end_date'])));
			}
			// Log the parsed dates for debugging
			log_message('debug', 'Sales date filter: start_date=' . $filters['start_date'] . ', end_date=' . $filters['end_date']);
		}
		...
		{
			// Recalculate totals for fixed tax items to display correctly
	...
	public function item_search()
	{
		...
		foreach ($suggestions as $suggestion) {
			if (isset($suggestion['value'])) {
				$existing_item_ids[] = $suggestion['value'];
			}
		}

		// Custom direct search for categories and subcategories that bypasses the normal search logic
		$this->db->select('items.item_id, items.name, items.item_number');
		$this->db->from('items');
		$this->db->join('ospos_item_categories', 'ospos_item_categories.id = items.item_category_id', 'left');
		$this->db->where('items.deleted', 0);
		$this->db->like('ospos_item_categories.name', $search);
		$category_results = $this->db->get()->result();

		foreach($category_results as $row) {
			// Only add if this item isn't already in the suggestions
			if (!in_array($row->item_id, $existing_item_ids)) {
				$suggestions[] = array('value' => $row->item_id, 'label' => $row->name);
				$existing_item_ids[] = $row->item_id; // Add to our tracking array
			}
		}

		// Also search by subcategory
		$this->db->select('items.item_id, items.name, items.item_number');
		$this->db->from('items');
		$this->db->join('ospos_item_subcategories', 'ospos_item_subcategories.id = items.item_subcategory_id', 'left');
		$this->db->where('items.deleted', 0);
		$this->db->like('ospos_item_subcategories.name', $search);
		$subcategory_results = $this->db->get()->result();

		foreach($subcategory_results as $row) {
			// Only add if this item isn't already in the suggestions
			if (!in_array($row->item_id, $existing_item_ids)) {
				$suggestions[] = array('value' => $row->item_id, 'label' => $row->name);
				$existing_item_ids[] = $row->item_id; // Add to our tracking array
		...
		// Debug info
		$debug_info = "Debug search: " . $search . "\n";
		$debug_info .= "Number of suggestions: " . count($suggestions) . "\n";
		if (count($suggestions) > 0) {
			$debug_info .= "First item: " . json_encode($suggestions[0]) . "\n";
		}
	...
	public function add()
	{
		...
		elseif($this->Item_kit->is_valid_item_kit($item_id_or_number_or_item_kit_or_receipt))
		{
			...
			elseif($stock_warning == $this->lang->line('sales_quantity_less_than_zero'))
			{
				$data['error'] = $stock_warning;
			}
		}
	...
	public function complete()
	{
		...
		if($this->sale_lib->is_invoice_mode())
		{
			...
			else
			{
				...
				$tax_details_var = (isset($tax_details) && is_array($tax_details) && count($tax_details) === 2) ? $tax_details : array(array(), array());
				$data['sale_id_num'] = $this->Sale->save(
					...
					$customer_id,
					$employee_id,
					$this->sale_lib->get_delivery_man_id(), // delivery_man_id in correct position
					$data['comments'],
					$invoice_number,
					$work_order_number,
					$quote_number,
				...
				else
				{
					...
					redirect('sales/receipt/' . $data['sale_id_num']);
				}
			}
		}
	...
	private function _load_sale_data($sale_id)
	{
		...
		// Fetch payments with payment_time for this sale
		$CI =& get_instance();
		$CI->load->model('Sale');
		$payments_query = $CI->Sale->get_sale_payments($sale_id);
		$data['payments'] = array();
		foreach ($payments_query->result_array() as $payment) {
			$data['payments'][] = $payment;
		}

		// --- DELIVERY MAN PATCH ---
		// Load delivery_man_id from the sale_info (database), not just from session
		$data['delivery_man_id'] = isset($sale_info['delivery_man_id']) ? $sale_info['delivery_man_id'] : null;
		// Load employees for lookup
		$this->load->model('Employee');
		$data['employees'] = $this->Employee->get_all(10000, 0, true)->result_array();
		// Add delivery man details if set
	...
	private function _reload($data = array())
	{
		...
		// Make sure tax_option property is set for each item
		foreach($data['cart'] as $line => $item)
		{
			if(!isset($item['tax_option']))
			{
				// Load the item tax_option from the database if not set
				$item_info = $this->Item->get_info($item['item_id']);
				$data['cart'][$line]['tax_option'] = isset($item_info->tax_option) ? $item_info->tax_option : 'without_tax';
			}
		}
		...
		// Add delivery man id to data for the view
		$data['delivery_man_id'] = $this->sale_lib->get_delivery_man_id();
	...
	public function receipt($sale_id)
	{
		...
		// Allow embedded rendering without header/print scripts when requested
		$data['embed'] = $this->input->get('embed') ? TRUE : FALSE;

		// Add delivery man details to $data for the receipt view
		if (!empty($data['delivery_man_id']) && !empty($data['employees'])) {
			foreach ($data['employees'] as $employee) {
				if ($employee['person_id'] == $data['delivery_man_id']) {
					$data['delivery_man_name'] = $employee['first_name'] . ' ' . $employee['last_name'];
	...
	public function edit($sale_id)
	{
		...
		$sale_info_object = $this->Sale->get_info($sale_id)->row();
		
		// Recalculate totals for proper fixed tax handling
		$recalculated_sale = $this->Sale->recalculate_fixed_tax_totals($sale_info_object);
		
		// Convert to array for the view
		$sale_info = $this->xss_clean((array)$sale_info_object);
		$sale_info['subtotal'] = $recalculated_sale->subtotal;
		$sale_info['tax'] = $recalculated_sale->tax;
		$sale_info['total'] = $recalculated_sale->total;
		...
		$balance_due = round($sale_info['total'] - $sale_info['amount_tendered'] + $sale_info['cash_refund'], totals_decimals(), PHP_ROUND_HALF_UP);
	...
	public function save($sale_id = -1)
	{
		...
		if($payment_type != PAYMENT_TYPE_UNASSIGNED && $payment_amount <> 0)
		{
			...
			else
			{
				...
				// Recalculate total for proper fixed tax handling
				$recalculated_sale = $this->Sale->recalculate_fixed_tax_totals($sale_info_object);
				$total_due = $recalculated_sale->total;

				if($amount_tendered > $total_due)
				{
					$cash_refund = $amount_tendered - $total_due;
				}
			}
	...
	public function suspend()
	{
		...
		$data = array();
		// Calculate taxes for the cart before saving
	...
	// Print a suspended sale as a receipt
	public function print_suspended($sale_id)
	{
		...
		// Get all sale data (cart, payments, customer, etc.)
		$sale_info = $this->Sale->get_info($sale_id)->row_array();
		if (!$sale_info || $sale_info['sale_status'] != SUSPENDED) {
			show_error('Suspended sale not found.');
		}

		// Load cart items (no item kits for suspended sales)
	...
	// Show the suspended sale receipt in the same way as the complete sale receipt
	public function suspended_receipt($sale_id)
	{
		$data = $this->_load_sale_data($sale_id);
		$data['is_suspended'] = TRUE;
		...
		if ($show_tax === '0') {
			$data['receipt_template'] = 'receipt_suspended_no_tax';
		} else {
			$data['receipt_template'] = 'receipt_suspended';
		}

		// Use the standard printable wrapper for receipts
		$this->load->view('sales/receipt', $data);
	}

	public function set_delivery_man()
	{
		$delivery_man_id = $this->input->post('delivery_man_id');
		$this->sale_lib->set_delivery_man_id($delivery_man_id);
		$this->_reload();
