<div id="receipt_wrapper" style="font-size:<?php echo $this->config->item('receipt_font_size');?>px">
	...
	<div id="receipt_general_info">
		...
		<div id="employee">Employee: <?php echo $employee; ?></div>
		<?php
		if (empty($delivery_man_name) && !empty($delivery_man_id) && !empty($employees)) {
			foreach ($employees as $employee) {
				if ($employee['person_id'] == $delivery_man_id) {
					$delivery_man_name = $employee['first_name'] . ' ' . $employee['last_name'];
					$delivery_man_phone = $employee['phone_number'];
					$delivery_man_email = $employee['email'];
					break;
				}
			}
		}
		if (!empty($delivery_man_name)) { ?>
				...
				<tr>
					...
					<?php 
						if(isset($tax['tax_type']) && $tax['tax_type'] == 1) {
							// Fixed amount tax
							echo to_currency($tax['tax_rate']) . ' ' . $tax['tax_group']; 
						} else {
							// Percentage tax
