...
<div id="title_bar" class="print_hide btn-toolbar" style="margin-bottom:10px;">
    <a href="<?php echo site_url('receivings/manage'); ?>" class="btn btn-info btn-sm pull-right">
        <span class="glyphicon glyphicon-list-alt">&nbsp;</span>Manage Receivings
    </a>
	...
	<table class="sales_table_100" id="register">
		<thead>
			<tr>
				...
				<!-- Ship pack column hidden - default value set to 1 -->
			...
			?>
					<?php echo form_open($controller_name."/edit_item/$line", array('class'=>'form-horizontal', 'id'=>'cart_'.$line)); ?>
						<tr>
							...
							<!-- Ship pack hidden with default value 1 -->
							<?php echo form_hidden('receiving_quantity', 1); ?>
							...
							?>
                                ...
                                <td>
                                    <div class="input-group">
                                        <?php echo form_input(array('name'=>'tax', 'class'=>'form-control input-sm', 'value'=>isset($item['tax_type']) && $item['tax_type'] ? to_currency_no_money(isset($item['tax'])?$item['tax']:0) : to_decimals(isset($item['tax'])?$item['tax']:0), 'onClick'=>'this.select();')); ?>
                                        <span class="input-group-btn">
                                            <?php echo form_checkbox(array('id'=>'tax_toggle', 'name'=>'tax_toggle', 'value'=>1, 'data-toggle'=>"toggle",'data-size'=>'small', 'data-onstyle'=>'success', 'data-on'=>'<b>'.$this->config->item('currency_symbol').'</b>', 'data-off'=>'<b>%</b>', 'data-line'=>$line, 'checked'=>isset($item['tax_type']) ? $item['tax_type'] : 0)); ?>
                                        </span>
                                    </div>
                                </td>
							<?php
                            ...
                            <td>
                            <?php 
                                $extended = $item['price']*$item['quantity']*$item['receiving_quantity'];
                                ...
                                $tax_amount = (isset($item['tax_type']) && $item['tax_type'] == PERCENT) ? ($after_discount * (isset($item['tax'])?$item['tax']:0) / 100) : (isset($item['tax'])?$item['tax']:0);
                                echo to_currency($after_discount + $tax_amount);
                            ?></td> 
...
$(document).ready(function()
{
    ...
    // Single-payment flow: no add payment button
	...
	$('[name="discount_toggle"]').change(function() {
        ...
        $('[name="tax_toggle"]').change(function() {
            var input = $("<input>").attr("type", "hidden").attr("name", "tax_type").val(($(this).prop('checked'))?1:0);
            $('#cart_'+ $(this).attr('data-line')).append($(input));
            $('#cart_'+ $(this).attr('data-line')).submit();
        });
