...
function get_sales_manage_table_headers()
{
	...
	$headers = array(
		...
		// array('total_discount' => $CI->lang->line('sales_discount')),
		array('tax' => $CI->lang->line('sales_tax')),
		array('total' => $CI->lang->line('sales_total')),
...
*/
function get_sale_data_row($sale, $controller_name = null)
{
	$CI =& get_instance();
    ...
    $row = array (
		...
		'subtotal' => to_currency($sale->subtotal),
		// 'total_discount' => isset($sale->total_discount) ? to_currency($sale->total_discount) : to_currency(0),
		'tax' => isset($sale->tax) ? to_currency($sale->tax) : to_currency(0),
	...
	// Only add the edit (with delete) button if not in the customers sales context
	if ($controller_name !== 'sales' || $CI->uri->segment(2) !== 'sales') {
...
function get_sale_data_last_row($sales)
{
	...
	{
		// Clean values before numeric operations
		$subtotal = floatval(str_replace([',', '€', '$', '£'], '', strip_tags($sale->subtotal)));
		$amount_tendered = floatval(str_replace([',', '€', '$', '£'], '', strip_tags($sale->amount_tendered)));
		$change_due = floatval(str_replace([',', '€', '$', '£'], '', strip_tags($sale->change_due)));
		$tax = isset($sale->tax) ? floatval(str_replace([',', '€', '$', '£'], '', strip_tags($sale->tax))) : 0;
		$total = floatval(str_replace([',', '€', '$', '£'], '', strip_tags($sale->total)));

		$sum_subtotal += $subtotal;
		$sum_amount_tendered += $amount_tendered;
		$sum_change_due += $change_due;
		$sum_tax += $tax;
		$sum_total += $total;
	}
	...
	return array(
		...
		'subtotal' => '<b>'.to_currency($sum_subtotal).'</b>',
		'tax' => '<b>'.to_currency($sum_tax).'</b>',
		'total' => '<b>'.to_currency($sum_total).'</b>',
...
/*
Get the header for the delivery men tabular view
*/
function get_delivery_men_manage_table_headers()
{
	$CI =& get_instance();
	...
	$headers = array(
		...
		array('email' => $CI->lang->line('common_email')),
		array('phone_number' => $CI->lang->line('common_phone_number')),
		array('username' => $CI->lang->line('delivery_men_username')),
		array('sales' => 'Sales', 'sortable' => FALSE)
	);
...
/*
Get the html data row for the delivery man
*/
function get_delivery_man_data_row($person)
{
	$CI =& get_instance();
	...
	$row = array (
		...
		'sales' => anchor("delivery_men/sales/$person->person_id", '<span class="glyphicon glyphicon-list-alt"></span>', array('class'=>'btn btn-info btn-xs', 'title'=>'View Sales')),
		'messages' => empty($person->phone_number) ? '' : anchor("Messages/view/$person->person_id", '<span class="glyphicon glyphicon-phone"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line('messages_sms_send'))),
		'edit' => anchor($controller_name."/view/$person->person_id", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update'))
...
function get_items_manage_table_headers()
{
	...
	$CI->load->model('ItemAttribute');
	$definition_names = $CI->ItemAttribute->get_definitions_by_flags(ItemAttribute::SHOW_IN_ITEMS);
...
function get_item_data_row($item)
{
	...
	else
	{
		...
		{
			if(isset($tax_info['tax_type']) && $tax_info['tax_type'] == 1) {
				// Fixed amount tax
				$tax_percents .= to_currency($tax_info['percent']) . ', ';
			} else {
				// Percentage tax
...
function get_item_category_manage_table_headers()
{
	$CI =& get_instance();
	$headers = array(
		array('id' => $CI->lang->line('item_categories_id')),
		array('name' => $CI->lang->line('item_categories_name')),
		array('description' => $CI->lang->line('item_categories_description'))
	);
	return transform_headers($headers);
}

function get_item_category_data_row($category)
{
	$CI =& get_instance();
	$controller_name = strtolower(get_class($CI));
	return array(
		...
		'edit' => anchor($controller_name . "/view/$category->id", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update'))
		)
	);
}

function get_item_subcategory_manage_table_headers()
{
	$CI =& get_instance();
	$headers = array(
		array('id' => $CI->lang->line('item_subcategories_id')),
		array('category_id' => $CI->lang->line('item_subcategories_category_id')),
		array('name' => $CI->lang->line('item_subcategories_name')),
		array('description' => $CI->lang->line('item_subcategories_description'))
	);
	return transform_headers($headers);
}

function get_item_subcategory_data_row($subcategory)
{
	$CI =& get_instance();
	$controller_name = strtolower(get_class($CI));
	return array(
