...
class Inventory_summary extends Report
{
	...
	{
		$this->db->select($this->Item->get_item_name('name') . ', items.item_number, item_categories.name as category, item_quantities.quantity, (item_quantities.quantity * items.qty_per_pack) as low_sell_quantity, items.reorder_level, stock_locations.location_name, items.cost_price, items.unit_price, (items.cost_price * item_quantities.quantity) AS sub_total_value');
		...
		if($query === FALSE)
		{
			error_log('Inventory_summary getData query failed: ' . print_r($this->db->error(), TRUE));
			return array();
		}
