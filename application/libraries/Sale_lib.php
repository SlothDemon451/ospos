...
class Sale_lib
{
	...
	public function get_totals($taxes)
	{
        ...
        foreach ($cart as $item) {
			...
			// Calculate prediscount subtotal (before any discounts)
			$prediscount_subtotal += $quantity * $unitPrice;
			
            // Get tax information for this item
			$tax_info = $this->CI->Item_taxes->get_info($item['item_id']);
			
			// Calculate item total with tax first (before discount)
			$item_total_with_tax = $quantity * $unitPrice;
            $item_tax_amount = 0;
            $has_fixed_included = false;
            $fixed_base_per_unit = 0;
			...
			if (!empty($tax_info)) {
				foreach ($tax_info as $tax) {
                    ...
                    if ($tax_type == 1) {
                        // Fixed-style tax (included): split unit price into base + tax
                        $fixed_base_per_unit = $unitPrice / (1 + ($tax_percent / 100));
                        $item_tax_amount = ($unitPrice - $fixed_base_per_unit) * $quantity;
                        $item_total_with_tax = $quantity * $unitPrice; // includes tax
                        $has_fixed_included = true;
                    } else {
						// Percentage tax: add tax to the total
						$item_tax_amount = ($quantity * $unitPrice) * ($tax_percent / 100);
						$item_total_with_tax += $item_tax_amount;
					}
				}
			}
			
			// Now apply discount to the total amount including tax
			if ($discount > 0) {
				if ($discount_type == PERCENT) {
					$discount_amount = $item_total_with_tax * ($discount / 100);
				} else {
					$discount_amount = $discount * $quantity;
				}
				$item_total_with_tax -= $discount_amount;
			}
			
            // Split subtotal and tax for summary
            if ($has_fixed_included) {
                // Base is the fixed base per unit times quantity (no discount is expected on included tax portion in this simple model)
                $final_tax = $item_tax_amount;
                $final_subtotal = $item_total_with_tax - $final_tax; // equals fixed_base_per_unit * qty after discount (if any)
            } else {
                // Percentage taxes: split proportionally
                $original_tax_ratio = ($quantity * $unitPrice + $item_tax_amount) > 0 ? ($item_tax_amount / ($quantity * $unitPrice + $item_tax_amount)) : 0;
                $final_tax = $item_total_with_tax * $original_tax_ratio;
                $final_subtotal = $item_total_with_tax - $final_tax;
            }
		...
		$total_discount = $prediscount_subtotal - $subtotal;
		...
		$totals['item_count'] = count($cart);
		$totals['total_units'] = 0;
		foreach ($cart as $item) {
			$totals['total_units'] += $item['quantity'];
		}
		$totals['cash_adjustment_amount'] = 0;

		if($totals['payments_cover_total'])
		{
			$totals['cash_adjustment_amount'] = round($cash_amount_due - $totals['total'], totals_decimals(), PHP_ROUND_HALF_UP);
		}
	...
	public function add_item(&$item_id, $quantity = 1, $item_location, &$discount = 0.0, $discount_type = 0, $price_mode = PRICE_MODE_STANDARD, $kit_price_option = NULL, $kit_print_option = NULL, $price_override = NULL, $description = NULL, $serialnumber = NULL, $sale_id = NULL, $include_deleted = FALSE, $print_option = NULL, $line = NULL)
	{
		...
		$attribute_links = $this->CI->ItemAttribute->get_link_values($item_id, 'sale_id', $sale_id, ItemAttribute::SHOW_IN_SALES)->row_object();
		...
		if(!$itemalreadyinsale || $item_info->is_serialized)
		{
			$item = array($insertkey => array(
					...
					'category' => isset($item_info->category) ? $item_info->category : '',
					'item_category_id' => isset($item_info->item_category_id) ? $item_info->item_category_id : '',
					'item_subcategory_id' => isset($item_info->item_subcategory_id) ? $item_info->item_subcategory_id : '',
					'tax_option' => isset($item_info->tax_option) ? $item_info->tax_option : 'without_tax',
				)
			);
			//add to existing array
			$items += $item;
	...
	public function set_delivery_man_id($delivery_man_id)
	{
		$this->CI->session->set_userdata('delivery_man_id', $delivery_man_id);
	}

	public function get_delivery_man_id()
	{
		return $this->CI->session->userdata('delivery_man_id');
	}

	public function clear_delivery_man()
	{
		$this->CI->session->unset_userdata('delivery_man_id');
	}
}
