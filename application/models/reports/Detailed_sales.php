...
class Detailed_sales extends Report
{
	...
	public function getData(array $inputs)
	{
		...
		$data = array();
		$summary_query = $this->db->get();
		if($summary_query === FALSE)
		{
			log_message('error', 'Detailed_sales getData summary query failed: ' . print_r($this->db->error(), TRUE));
			return array('summary' => array(), 'details' => array(), 'rewards' => array());
		}
		$data['summary'] = $summary_query->result_array();
		$data['details'] = array();
		$data['rewards'] = array();
		...
		foreach($data['summary'] as $key=>$value)
		{
			...
			{
				log_message('error', 'Detailed_sales getData details query failed: ' . print_r($this->db->error(), TRUE));
				$data['details'][$key] = array();
			}
			else
			{
				$data['details'][$key] = $details_query->result_array();
			}

			$this->db->select('used, earned');
			$this->db->from('sales_reward_points');
			$this->db->where('sale_id', $value['sale_id']);
			$rewards_query = $this->db->get();
			if($rewards_query === FALSE)
			{
				log_message('error', 'Detailed_sales getData rewards query failed: ' . print_r($this->db->error(), TRUE));
				$data['rewards'][$key] = array();
			}
			else
			{
				$data['rewards'][$key] = $rewards_query->result_array();
			}
		}
